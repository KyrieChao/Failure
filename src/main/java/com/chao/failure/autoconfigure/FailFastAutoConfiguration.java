...
import com.chao.failure.config.registry.*;
import com.chao.failure.integration.mvc.DefaultExceptionHandler;
import com.chao.failure.integration.mvc.FailFastExceptionHandler;
...
public class FailFastAutoConfiguration {
    ...
    @Bean
    public FrameworkDefaultConfigurer frameworkDefaultConfigurer() {
        return new FrameworkDefaultConfigurer();
    }

    @Bean
    @ConditionalOnMissingBean
    public SkipTypeRegistry skipTypeRegistry(ObjectProvider<FailFastConfigurer> configurers) {
        DefaultSkipTypeRegistry registry = new DefaultSkipTypeRegistry();
        configurers.orderedStream().forEach(configurer -> configurer.addValidationSkipTypes(registry));
        if (log.isDebugEnabled()) {
            log.debug("[FailFast] SkipTypeRegistry initialized");
        }
        return registry;
    }
    ...
    @Bean
    @ConditionalOnMissingBean
    public SkipPrefixRegistry skipPrefixRegistry(ObjectProvider<FailFastConfigurer> configurers) {
        DefaultSkipPrefixRegistry registry = new DefaultSkipPrefixRegistry();
        configurers.orderedStream().forEach(configurer -> configurer.addExceptionSkipPrefixes(registry));
        if (log.isDebugEnabled()) {
            log.debug("[FailFast] SkipPrefixRegistry initialized");
        }
        return registry;
    }
    ...
    @Bean
    @ConditionalOnMissingBean
    public ValidatorRegistry validatorRegistry(ObjectProvider<FailFastConfigurer> configurers) {
        DefaultValidatorRegistry registry = new DefaultValidatorRegistry();
        configurers.orderedStream().forEach(configurer -> configurer.addCustomValidators(registry));
        if (log.isDebugEnabled()) {
            log.debug("[FailFast] ValidatorRegistry initialized");
        }
        return registry;
    }
